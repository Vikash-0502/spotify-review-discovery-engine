...
from models.schema import Quote, Review, WeeklyPulse  # noqa: E402
...
def test_audit_reviews_flags_pii(audit_db):
    _insert_review(audit_db, content="Contact me at leaked@example.com about shuffle bugs", external_id="ext-pii")
    report = audit_reviews(audit_db)
    assert report.passed is False
    assert any("email" in finding.issue for finding in report.findings)


def test_audit_reviews_flags_bad_author_format(audit_db):
    _insert_review(
        audit_db,
        content="Shuffle keeps repeating the same songs.",
        author="real_username",
        external_id="ext-bad-author",
    )
    report = audit_reviews(audit_db)
    assert report.passed is False
...
def test_audit_database_merges_surface_counts(audit_db):
    from models.schema import RawReview, Theme

    review = _insert_review(audit_db, content="I wish search surfaced more new artists.")
    theme = Theme(
        id="theme-1",
        name="Search gaps",
    ...
    report = audit_database(audit_db)
